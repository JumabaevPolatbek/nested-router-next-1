...
export const getServerSideProps: GetServerSideProps<{
  ...
  const requestPost = await fetch("https://jsonplaceholder.typicode.com/posts");
  const dataPosts: Post[] = await requestPost.json();
  return {
    props: {
      user: dataUser,
      posts: dataPosts.filter((post) => post.userId === +id),
    },
  };
};
...
const Index = ({ user, posts }: { user: User; posts: Post[] }) => {
  ...
  return (
    <Paper elevation={2} className="mt-2 py-4 px-3 bg-gray-400">
      ...
      <h3 className="text-center text-[24px] text-bold">{name}</h3>
      ...
      <ul>
        <div className="text-center text-[18px] text-bold">Posts</div>
        {posts.map((post) => (
          <li key={post.id}>
            <Link
              href={{
